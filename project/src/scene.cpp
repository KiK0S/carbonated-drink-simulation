...
void scene_structure::initialize()
{
	...
	bubble.initialize_data_on_gpu(curve_primitive_circle(0.2));
	bubble.color = { 0,0,0 };
}
...
void scene_structure::display_frame()
{
	...
	bubble.model.scaling = sph_parameters.h;
...
void update_field_color(grid_2D<vec3>& field, numarray<particle_element> const& particles)
{
	...
	for (int kx = 0; kx < Nf; ++kx) {
		for (int ky = 0; ky < Nf; ++ky) {
			...
			field(kx, Nf - 1 - ky) =  vec3(1,1,1) - clamp(f, 0, 1) * vec3(0.02, 0.31, 0.91);
		}
	}
}
